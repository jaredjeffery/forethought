...
The product UI should call forecastable series **Indicators**. Keep `variables` as the internal table/route name until a route/schema rename is worth the migration cost. The `/variables` public directory should be labelled "Indicators" and support filters for country, category, and frequency (annual/quarterly/monthly).
